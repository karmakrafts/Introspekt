...
internal class SourceLocationTransformer( // @formatter:off
...
) : TrakkitIntrinsicTransformer<IrSimpleFunction?>( // @formatter:on
    setOf(
    ...
    override fun visitIntrinsic(
        ...
        when (type) { // @formatter:off
            TrakkitIntrinsics.SL_HERE ->
                getCallLocation(moduleFragment, file, source, expression, data).instantiate()
            TrakkitIntrinsics.SL_HERE_HASH ->
                getCallLocation(moduleFragment, file, source, expression, data).createHashSum()
            TrakkitIntrinsics.SL_CURRENT_FUNCTION ->
                getFunctionLocation(moduleFragment, file, source, data).instantiate()
            TrakkitIntrinsics.SL_CURRENT_FUNCTION_HASH ->
                getFunctionLocation(moduleFragment, file, source, data).createHashSum()
